...
function parsePem(pem) {
	...
	if (match)
		return parseRfc5915(Buffer.from(match[1].replace(/[\s-]/mg, ''), 'base64'))
	match = pem.match(pemPkcs8RE)
	if (match)
		return parsePkcs8(Buffer.from(match[1].replace(/[\s-]/mg, ''), 'base64'))
	match = pem.match(pemSpkiRE)
	if (match)
		return parseSpki(Buffer.from(match[1].replace(/[\s-]/mg, ''), 'base64'))
...
function ECKey(key, format) {
	...
	// STRING KEYS: base64 all the time, but also allowed in PEM
		...
		if (format == 'pem') {
			...
			const k = parsePkcs8(Buffer.from(key, 'base64'))
			curve = k.c
			x = k.x
			y = k.y
			d = k.d
			...
			const k = parseSpki(Buffer.from(key, 'base64'))
			curve = k.c
			x = k.x
			y = k.y
			d = k.d
	...
	// private key and "(publicKey|x,y)" always required (both for priv and pub)
		...
		if (util.isBuffer(key.publicKey)) {
			...
			const k = parsePublicKeyBuffer(curve, Buffer.from(key.publicKey, 'base64'))
			x = k.x
			y = k.y
